...
pub struct EditFileTool { client: RuntimeClient }
impl EditFileTool { pub fn new(client: RuntimeClient) -> Self { Self { client } } }
...
impl Tool for EditFileTool {
    ...
    async fn execute(&self, input: Value) -> Result<Value, ToolCallError> {
        ...
        let old_text = input["old_text"].as_str().ok_or_else(|| ToolCallError::InvalidInput("missing 'old_text'".into()))?.to_string();
        let new_text = input["new_text"].as_str().ok_or_else(|| ToolCallError::InvalidInput("missing 'new_text'".into()))?.to_string();
        self.client.invoke(ToolCall::EditFile(EditFileInput { path, old_text, new_text }))
