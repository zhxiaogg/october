...
pub struct WriteFileTool { client: RuntimeClient }
impl WriteFileTool { pub fn new(client: RuntimeClient) -> Self { Self { client } } }
...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, input: Value) -> Result<Value, ToolCallError> {
        ...
        let content = input["content"].as_str().ok_or_else(|| ToolCallError::InvalidInput("missing 'content'".into()))?.to_string();
        self.client.invoke(ToolCall::WriteFile(WriteFileInput { path, content }))
